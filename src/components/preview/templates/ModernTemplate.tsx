      ...
      <div className="print-break-avoid -mx-[15mm] -mt-[15mm] px-[15mm] pt-[15mm] pb-6 mb-6" style={{ backgroundColor: ac }}>
        <div className="flex justify-between items-start">
          <div>
            ...
            <DocTitle className="text-2xl font-bold text-white tracking-tight">{label}</DocTitle>
          ...
          {data.items.map((item, i) => {
            ...
            return (
              <tr key={item.id} className="border-b border-gray-100">
                ...
                <td className="py-2.5 px-3 text-xs text-gray-900">{formatDescription(item) || '—'}</td>
                <td className="py-2.5 px-3 text-xs text-gray-700 text-right">{formatQuantity(item, modeSpec(data.mode).defaultUnit)}</td>
