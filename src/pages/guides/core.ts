...
/**
 * The two foundational guides.
 *
 * They used to live in guideData.ts. That file statically imports every other
 * cluster to build `guideConfigs`, so a lazy import of these two pulled the
 * whole corpus into one chunk and defeated the split. Keeping them in their own
 * module lets guideData.ts stay what it now is: a build-time aggregator that
 * only the prerenderer loads.
 */
